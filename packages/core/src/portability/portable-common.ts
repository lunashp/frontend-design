/**
 * Shared machinery for turning a component's local import subtree into a mirrored,
 * copy-ready file set: path mirroring, per-file import rewriting, dependency
 * resolution (incl. installed peers), and the dangling-import check.
 *
 * Extracted so the single-component resolver (portability-resolver.ts) and the
 * multi-component kit resolver (resolve-many.ts) share ONE implementation — the
 * kit's shared token namespace only holds if it mirrors and rewrites files
 * exactly as the single-component path does; two divergent copies would drift.
 */
...
import { isDeferredImportCall } from '../graph/import-graph.js';
...
export const TEMPLATE_PROVIDED = new Set(['react', 'react-dom']);
...
/**
 * Resolve an import graph's externals into an install list: direct deps at their
 * declared range (or `latest`), plus installed peers, minus fully-stubbed
 * packages. Shared by the single-component and kit paths so both compute the
 * SAME range for a given package — the invariant the kit's conflict detection
 * relies on (a range difference must be a real requirement difference, never an
 * artefact of two divergent resolvers).
 */
export function resolveExternalDeps(
  externals: Iterable<string>,
  droppedPackages: ReadonlySet<string>,
  loaded: LoadedProject,
  rofs: ReadOnlyFs,
): Record<string, string> {
  ...
  for (const name of externals) {
    ...
    directDeps[name] = loaded.pkg.dependencies[name] ?? loaded.pkg.devDependencies[name] ?? 'latest';
  }
  return dropStubbed(withInstalledPeers(directDeps, loaded, rofs), droppedPackages);
}

export function commonBaseDir(files: readonly string[]): string {
...
export function bundlePathOf(abs: string, base: string): string {
 ...
 */
export function findDanglingImports(files: Record<string, string>): string[] {
