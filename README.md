...
2. **격리 렌더링** — 추출한 컴포넌트를 원본과 분리된 **Sandpack iframe**에서 실제로 렌더링해 눈으로 확인합니다.
...
(디스크 경로, 읽기 전용)        └ @ce/core 엔진 실행                 └ 컴포넌트를 Sandpack
                              (필요 소스를 .workspace/ 로 복사)      격리 iframe에 렌더
```
...
| 패키지     | 역할                                                                     |
| ---------- | ------------------------------------------------------------------------ |
| `@ce/core` | 프레임워크 무관 엔진. 스캔 → 분류 → 이식성 → 토큰화. **Sandpack을 import하지 않음** |
| `@ce/host` | `@ce/core`를 감싸는 로컬 HTTP + WebSocket 서버 (읽기 전용으로 대상 프로젝트 접근) |
| `@ce/web`  | Vite + React 갤러리 UI. host와 JSON API로만 통신                         |
| `@ce/mcp`  | (P5, 미구현) 에이전트용 MCP 서버 자리표시자                              |
...
**터미널 1 — 백엔드(host)**: 분석할 대상 프로젝트 경로를 지정해 실행합니다.

```bash
pnpm --filter @ce/host dev -- --project ../backoffice-frontend
# → http://localhost:4317 에서 실행
```

**터미널 2 — 갤러리(web)**:

```bash
pnpm --filter @ce/web dev
# → http://localhost:5173 접속
```

브라우저에서 **http://localhost:5173** 를 열면 됩니다. `--project`로 지정한 프로젝트가 자동으로 스캔됩니다.
...
```bash
pnpm --filter @ce/host dev -- --project <경로> [--port 4317] [--workspace <워크스페이스 경로>]
```

| 옵션          | 설명                                              | 기본값             |
| ------------- | ------------------------------------------------- | ------------------ |
| `--project`   | 분석할 대상 프로젝트 경로 (첫 인자로도 전달 가능) | (없으면 UI에서 입력) |
| `--port`      | host 서버 포트                                    | `4317`             |
...
추출한 컴포넌트를 **격리된 Sandpack iframe**에서 실제로 렌더링합니다.
...
│  │  │  ├─ sandbox/        # SandpackSpec 조립 (엔트리/샘플 props)
...
- **렌더링**: 엔진은 직렬화 가능한 `SandpackSpec`만 내보내고, 웹이 이를 `@codesandbox/sandpack-react`로 렌더합니다.
...
- ⬜ **P5 — MCP 서버**: 같은 엔진(`EngineSession`)을 에이전트용 MCP 도구(`scan_project` / `list_components` / `get_portable_code` / `customize_component`)로 감싸는 작업. 엔진이 이미 transport-무관이라 얇은 어댑터로 구현됩니다.
...
- 이 워크스페이스는 아직 **git 저장소가 아닙니다**. `git init` 후 사용하면 되고, `.gitignore`와 CI 워크플로우는 이미 준비되어 있습니다.
- 브라우저 자동화 확장(Playwright/claude-in-chrome)이 이 환경엔 연결돼 있지 않아, 개발 중 시각 검증은 headless `playwright`로 진행했습니다.
- 대상이 pnpm 모노레포라 내부 패키지가 `workspace:*`로 고정돼 있으면, 그 패키지를 쓰는 컴포넌트는 샌드박스에서 설치가 안 되므로 "code-only"로 표시됩니다(의도된 안전 동작).
